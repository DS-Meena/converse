...
class PublicChatConsumer(AsyncJsonWebsocketConsumer):
	...
	async def connect(self):
		...
		# accept connection to that room
		await self.accept()
		self.room_id = None
	...
	async def receive_json(self, content):
		...
		try:
			# now take action according to given command

			if command == "send":
				if len(content["message"].lstrip()) != 0:
					# call send room function (to send message)
...
# get no of total connected users
def get_num_connected_users(room):
	if room.users:
		return len(room.users.all())
	return 0

# DATABASE WORK SYNC WITH IT
